...
import { GET_LISTINGS_BY_TENANT } from '@/components/queries/queries';
...
export default function Page() {
  ...
  const { data, loading, error, refetch } = useQuery(GET_LISTINGS_BY_TENANT, {
    variables: {
      id: [userId],
    },
    skip: !userId,
        ...
        {!loading && data?.getListings?.listings.length === 0 ? (
          <>
            ...
            <div className="account__column-inner">
              ...
              <h4>¡Todavía no tenés ningún alquiler!</h4>
            </div>
          </>
        ) : (
          <>
