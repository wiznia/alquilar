import React from 'react';

export const icons = {
  logo: (props) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="102" height="19" fill="none" {...props}>
      <path fill="#1E2B39" d="M7.572.853H6.536L0 15.062h1.116L7.044 1.989l5.947 13.073h1.117L7.572.852Z"/>
      <path fill="#2C3947" d="M17.772 0v15.062h1.015V0h-1.015Z"/>
      <path fill="#3B4755" d="M33.913 4.466h-.994v2.497c-.426-.812-1.015-1.462-1.766-1.909-.772-.446-1.624-.67-2.578-.67a5.37 5.37 0 0 0-2.68.69 4.874 4.874 0 0 0-1.887 1.909c-.447.812-.67 1.746-.67 2.78 0 1.056.223 1.99.67 2.802a4.746 4.746 0 0 0 1.887 1.908c.792.447 1.685.67 2.68.67.934 0 1.786-.223 2.537-.67a4.798 4.798 0 0 0 1.787-1.867V19h1.014V4.466Zm-3.105 9.195c-.65.386-1.36.569-2.172.569s-1.543-.183-2.193-.569a3.82 3.82 0 0 1-1.522-1.583c-.365-.67-.548-1.441-.548-2.314 0-.853.183-1.624.548-2.314a4.077 4.077 0 0 1 1.522-1.584 4.407 4.407 0 0 1 2.193-.568 4.33 4.33 0 0 1 2.172.568c.65.386 1.157.914 1.522 1.584.366.69.569 1.461.569 2.314 0 .873-.203 1.644-.569 2.314a3.82 3.82 0 0 1-1.522 1.583Z"/>
      <path fill="#495463" d="M48.899 4.466h-1.015v5.643c0 1.258-.365 2.274-1.056 3.004-.69.751-1.644 1.117-2.821 1.117-1.117 0-1.97-.305-2.578-.934-.61-.609-.914-1.522-.914-2.72v-6.11H39.5v6.191c0 1.462.386 2.558 1.178 3.33.771.77 1.867 1.156 3.248 1.156.933 0 1.745-.203 2.435-.609.69-.406 1.198-.995 1.564-1.745v2.273h.974V4.466Z"/>
      <path fill="#576271" d="M54.63 4.466v10.596h1.016V4.466H54.63Zm-.08-2.801c.162.162.345.223.588.223a.813.813 0 0 0 .589-.244.78.78 0 0 0 .243-.588.768.768 0 0 0-.243-.569.813.813 0 0 0-.589-.243.78.78 0 0 0-.589.243.813.813 0 0 0-.243.589.78.78 0 0 0 .243.589Z"/>
      <path fill="#65707E" d="M61.353 0v15.062h1.015V0h-1.015Z"/>
      <path fill="#747E8C" d="M67.344 14.9a.826.826 0 0 0 .61.243.813.813 0 0 0 .588-.243.89.89 0 0 0 .264-.63.844.844 0 0 0-.264-.609.813.813 0 0 0-.588-.243.826.826 0 0 0-.61.243.826.826 0 0 0-.243.61c0 .243.081.466.243.629Z"/>
      <path fill="#828B9A" d="M79.724.853H78.69l-6.536 14.209h1.116l5.927-13.073 5.948 13.073h1.117L79.724.852Z"/>
      <path fill="#9099A8" d="M97.74 10.089c1.056-.305 1.888-.833 2.456-1.604.569-.771.873-1.705.873-2.842 0-1.482-.507-2.659-1.522-3.512C98.532 1.28 97.13.853 95.345.853H90.25v.933h5.095c1.502 0 2.66.345 3.471 1.015.792.67 1.198 1.624 1.198 2.842 0 1.238-.406 2.192-1.198 2.862-.812.67-1.969.995-3.471.995H90.25v5.562h1.035v-4.648h4.06c.406 0 .873-.02 1.38-.102l3.39 4.75h1.178l-3.553-4.973Z"/>
    </svg>
  ),
  search: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="16"
      height="16"
      fill="none"
      {...props}
    >
      <g clipPath="url(#a)">
        <path
          fill="#fff"
          fillRule="evenodd"
          d="m11.46 10.319 4.304 4.304a.806.806 0 1 1-1.141 1.14l-4.304-4.303a6.4 6.4 0 1 1 1.14-1.141h.002Zm-5.06.88a4.8 4.8 0 1 0 0-9.6 4.8 4.8 0 0 0 0 9.6Z"
          clipRule="evenodd"
        />
      </g>
      <defs>
        <clipPath id="a">
          <path fill="#fff" d="M0 0h16v16H0z" />
        </clipPath>
      </defs>
    </svg>
  ),
  notificationUser: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="31"
      height="31"
      fill="none"
      {...props}
    >
      <rect
        width="30"
        height="29"
        fill="#FF9500"
        rx="14.5"
        transform="matrix(.99998 -.00315 -.00173 1.00002 .084 1.055)"
      />
      <path
        fill="#FAFAFA"
        d="M15.061 14.008a3.48 3.48 0 0 1-3.494-3.49 3.52 3.52 0 0 1 3.506-3.51 3.48 3.48 0 0 1 3.494 3.489 3.52 3.52 0 0 1-3.506 3.51ZM8.047 22.28l-.002.875a.87.87 0 0 0 .874.872l12.25-.038a.88.88 0 0 0 .876-.878l.002-.875a5.225 5.225 0 0 0-1.532-3.708 5.224 5.224 0 0 0-3.71-1.526l-3.5.011a5.275 5.275 0 0 0-3.714 1.55 5.276 5.276 0 0 0-1.544 3.717Z"
      />
    </svg>
  ),
  user: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="24"
      height="24"
      fill="none"
      {...props}
    >
      <circle cx="12" cy="12" r="12" fill="#FF9500" />
      <path
        fill="#fff"
        d="M12 11.5a3 3 0 1 0 0-6 3 3 0 0 0 0 6ZM18 17v.75a.75.75 0 0 1-.75.75H6.75a.75.75 0 0 1-.75-.75V17a4.5 4.5 0 0 1 4.5-4.5h3A4.5 4.5 0 0 1 18 17Z"
      />
    </svg>
  ),
  notification: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="24"
      height="24"
      fill="none"
      {...props}
    >
      <path
        fill="#FF9500"
        d="M13.7 20h-3.5c-.7 0-1.3.8-.9 1.5.6.9 1.6 1.5 2.7 1.5s2.1-.6 2.6-1.5c.4-.7-.1-1.5-.9-1.5ZM21.8 16.7l-.4-.5C19.8 14.1 19 11.6 19 9v-.7c0-3.6-2.6-6.8-6.2-7.2C8.6.6 5 3.9 5 8v1c0 2.6-.8 5.1-2.4 7.2l-.4.5c-.2.2-.3.6-.2.8.3.9 1.1 1.5 2 1.5h16c.9 0 1.7-.6 1.9-1.5.1-.3 0-.6-.1-.8Z"
      />
    </svg>
  ),
  arrowRight: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="12"
      height="11"
      fill="none"
      {...props}
    >
      <path
        fill="#fff"
        d="M0 5.5c0-.19.079-.371.22-.505a.77.77 0 0 1 .53-.21h8.688l-3.22-3.064a.698.698 0 0 1-.22-.506c0-.19.08-.371.22-.506A.77.77 0 0 1 6.75.5c.2 0 .39.075.531.21l4.5 4.284A.713.713 0 0 1 12 5.5a.685.685 0 0 1-.22.506l-4.5 4.285a.77.77 0 0 1-.53.209.77.77 0 0 1-.532-.21.698.698 0 0 1-.22-.505c0-.19.08-.372.22-.506l3.22-3.065H.75a.77.77 0 0 1-.53-.209A.697.697 0 0 1 0 5.5Z"
      />
    </svg>
  ),
  messageUser: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="67"
      height="68"
      fill="none"
      {...props}
    >
      <rect
        width="67"
        height="67"
        y=".5"
        fill="#FF9500"
        rx="33.5"
      />
      <path
        fill="#FAFAFA"
        d="M33.5 32.5a8.292 8.292 0 1 0 0-16.584 8.292 8.292 0 0 0 0 16.584Zm16.916 15.438v2.073c0 .55-.223 1.077-.62 1.466a2.136 2.136 0 0 1-1.495.607H18.698c-.56 0-1.098-.218-1.495-.607a2.053 2.053 0 0 1-.619-1.466v-2.073c0-3.299 1.337-6.462 3.716-8.795a12.817 12.817 0 0 1 8.97-3.643h8.459a12.81 12.81 0 0 1 8.97 3.643 12.316 12.316 0 0 1 3.717 8.795Z"
      />
    </svg>
  ),
  messageUserSmall: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="34"
      height="34"
      fill="none"
      {...props}
    >
      <rect width="34" height="34" fill="#FF9500" rx="17" />
      <path
        fill="#FAFAFA"
        d="M17 16a4 4 0 1 0 0-8 4 4 0 0 0 0 8ZM26 24v1c0 .265-.119.52-.33.707a1.2 1.2 0 0 1-.795.293H9.125a1.2 1.2 0 0 1-.795-.293A.947.947 0 0 1 8 25v-1c0-1.591.711-3.117 1.977-4.243C11.243 18.632 12.96 18 14.75 18h4.5c1.79 0 3.507.632 4.773 1.757C25.289 20.883 26 22.41 26 24Z"
      />
    </svg>
  ),
  messageRead: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      width="16"
      fill="#3b82f6"
      {...props}
    >
      <path d="M5.528 10.33a.501.501 0 0 0-.057.705l7.082 8.314a.504.504 0 0 0 .381.176h.016a.502.502 0 0 0 .384-.199L23.899 5.275a.5.5 0 1 0-.799-.601L12.91 18.226l-6.677-7.839a.5.5 0 0 0-.705-.057z" />
      <path d="m12.028 13.945 6.519-8.67a.5.5 0 1 0-.799-.601l-6.519 8.67a.5.5 0 1 0 .799.601zM.176 10.33a.501.501 0 0 0-.057.705l7.082 8.314a.504.504 0 0 0 .381.176h.016a.502.502 0 0 0 .384-.199l.967-1.285a.5.5 0 1 0-.799-.601l-.592.786-6.677-7.839a.5.5 0 0 0-.705-.057z" />
    </svg>
  ),
  sendMessage: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="28"
      height="28"
      fill="none"
      {...props}
    >
      <path
        fill="#fff"
        fillRule="evenodd"
        d="M26.89 3.638c.54-1.616-1.058-3.123-2.639-2.49l-.493.196L2.055 8.092c-1.28.398-1.435 2.144-.247 2.762l10.005 5.2c.032.017.065.033.098.047.014.033.03.065.046.097l5.206 9.995c.618 1.188 2.366 1.032 2.764-.246l6.73-21.612.234-.697ZM4.274 9.496a.2.2 0 0 0-.033.369l8.041 4.18a.2.2 0 0 0 .234-.037l9.706-9.697c.147-.148-.002-.394-.201-.332L4.273 9.496Zm14.248 14.235a.2.2 0 0 1-.368.033l-4.184-8.034a.2.2 0 0 1 .037-.234L23.712 5.8c.147-.148.394.001.332.2l-5.523 17.732Z"
        clipRule="evenodd"
      />
    </svg>
  ),
  heartFilled: (props) => (
    <svg
      width="25"
      height="25"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 122.88 107.41"
      {...props}
    >
      <path
        fill="#ff0000"
        d="M60.83 17.19C68.84 8.84 74.45 1.62 86.79.21c23.17-2.66 44.48 21.06 32.78 44.41-3.33 6.65-10.11 14.56-17.61 22.32-8.23 8.52-17.34 16.87-23.72 23.2l-17.4 17.26-14.38-13.84C29.16 76.9.95 55.93.02 29.95-.63 11.75 13.73.09 30.25.3c14.76.2 20.97 7.54 30.58 16.89z"
      />
    </svg>
  ),
  heart: (props) => (
    <svg
      width="25"
      height="25"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 122.88 109.57"
      {...props}
    >
      <path d="M65.46 19.57c-.68.72-1.36 1.45-2.2 2.32l-2.31 2.41-2.4-2.33c-.71-.69-1.43-1.4-2.13-2.09C49 12.58 43.41 7.08 31.9 6.93c-.45-.01-.93 0-1.43.02-6.44.23-12.38 2.6-16.72 6.65-4.28 4-7.01 9.67-7.1 16.57-.01.43 0 .88.02 1.37.69 19.27 19.13 36.08 34.42 50.01 2.95 2.69 5.78 5.27 8.49 7.88l11.26 10.85 14.15-14.04c2.28-2.26 4.86-4.73 7.62-7.37 4.69-4.5 9.91-9.49 14.77-14.52 3.49-3.61 6.8-7.24 9.61-10.73 2.76-3.42 5.02-6.67 6.47-9.57 2.38-4.76 3.13-9.52 2.62-13.97-.5-4.39-2.23-8.49-4.82-11.99a28.623 28.623 0 0 0-10.14-8.5C96.5 7.29 91.21 6.2 85.8 6.82c-9.33 1.08-14.3 6.35-20.34 12.75zm-4.69-4.72C67.67 7.54 73.4 1.55 85.04.22c6.72-.77 13.3.57 19.03 3.45 4.95 2.48 9.27 6.1 12.51 10.47 3.27 4.42 5.46 9.61 6.1 15.19.65 5.66-.29 11.69-3.3 17.69-1.7 3.39-4.22 7.03-7.23 10.76-2.95 3.66-6.39 7.44-10 11.17-4.95 5.13-10.21 10.17-14.95 14.71-2.77 2.65-5.36 5.13-7.54 7.29L63.2 107.28l-2.31 2.29-2.34-2.25-13.6-13.1c-2.49-2.39-5.37-5.02-8.36-7.75C20.38 71.68.81 53.85.02 31.77 0 31.23 0 30.67 0 30.09c.12-8.86 3.66-16.18 9.21-21.36C14.71 3.6 22.18.6 30.22.31c.55-.02 1.13-.03 1.74-.02C46 .48 52.42 6.63 60.77 14.85z" />
    </svg>
  ),
  contactUser: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="67"
      height="68"
      fill="none"
      {...props}
    >
      <rect width="67" height="67" y=".5" fill="#FF9500" rx="33.5" />
      <path
        fill="#FAFAFA"
        d="M33.5 32.5a8.292 8.292 0 1 0 0-16.584 8.292 8.292 0 0 0 0 16.584Zm16.916 15.438v2.073c0 .55-.223 1.077-.62 1.466a2.136 2.136 0 0 1-1.495.607H18.698c-.56 0-1.098-.218-1.495-.607a2.053 2.053 0 0 1-.619-1.466v-2.073c0-3.299 1.337-6.462 3.716-8.795a12.817 12.817 0 0 1 8.97-3.643h8.459a12.81 12.81 0 0 1 8.97 3.643 12.316 12.316 0 0 1 3.717 8.795Z"
      />
    </svg>
  ),
  contactPhone: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="20"
      height="20"
      fill="none"
      {...props}
    >
      <path
        fill="#FF9500"
        d="m19.969 15.117-.909 3.938a1.214 1.214 0 0 1-1.19.946C8.017 20 0 11.984 0 2.129c0-.573.39-1.063.946-1.19L4.884.031A1.226 1.226 0 0 1 6.28.74l1.817 4.238c.213.5.07 1.081-.35 1.424L5.645 8.09a13.659 13.659 0 0 0 6.23 6.23l1.722-2.101c.34-.421.925-.567 1.425-.35l4.238 1.816c.51.268.842.862.709 1.432Z"
      />
    </svg>
  ),
  eye: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="20"
      height="20"
      fill="none"
      viewBox="0 0 28 28"
      {...props}
    >
      <path
        fill="#737373"
        fillRule="evenodd"
        d="M17.747 15.415c.238-.54.372-1.143.372-1.413 0-2.348-1.824-4.25-4.073-4.25-2.25 0-4.073 1.902-4.073 4.25 0 2.347 2.037 3.887 4.073 3.887 1.303 0 2.462-.637 3.206-1.63a4.29 4.29 0 0 0 .495-.844Zm-3.701.349c.51 0 .975-.196 1.332-.518.432-.39.622-.965.622-1.244 0-1.174-.828-2.126-1.954-2.126-1.125 0-2.046.95-2.046 2.124 0 .81.92 1.764 2.046 1.764Z"
        clipRule="evenodd"
      />
      <path
        fill="#737373"
        fillRule="evenodd"
        d="M1.092 14.272a.448.448 0 0 1 0-.545c.01-.012.019-.025.028-.039C4.78 8.343 9.32 5.5 14.014 5.5c4.689 0 9.224 2.836 12.882 8.17.069.101.104.205.104.33 0 .13-.04.24-.114.346C23.23 19.67 18.698 22.5 14.014 22.5c-4.705 0-9.257-2.857-12.922-8.228Zm2.847-.92a1.04 1.04 0 0 0 0 1.294c3.135 3.898 6.665 5.729 10.075 5.729 3.41 0 6.94-1.83 10.075-5.729a1.04 1.04 0 0 0 0-1.294c-3.135-3.897-6.665-5.727-10.075-5.727-3.41 0-6.94 1.83-10.075 5.727Z"
        clipRule="evenodd"
      />
    </svg>
  ),
  eyeOff: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="20"
      height="20"
      fill="none"
      viewBox="0 0 28 28"
      {...props}
    >
      <path
        fill="#737373"
        fillRule="evenodd"
        d="M22.693 1.55a.794.794 0 0 0-1.093.297l-2.447 4.297c-1.667-.78-3.392-1.18-5.139-1.18-4.693 0-9.233 2.882-12.894 8.3l-.015.021-.012.019a.46.46 0 0 0 0 .552c2.7 4.013 5.884 6.641 9.255 7.746L8.4 25.022a.817.817 0 0 0 .293 1.108l.347.203a.794.794 0 0 0 1.092-.297L23.332 2.86a.817.817 0 0 0-.293-1.108l-.346-.203Zm-4.601 6.457c-1.357-.597-2.727-.888-4.078-.888-3.41 0-6.94 1.854-10.075 5.805-.3.38-.3.932 0 1.311 2.35 2.962 4.922 4.746 7.499 5.454l1.348-2.366c-1.54-.49-2.813-1.86-2.813-3.741 0-2.38 1.824-4.308 4.073-4.308 1.038 0 1.986.41 2.705 1.087l1.341-2.354Zm-2.453 4.307c-.346-.537-.916-.886-1.593-.886-1.125 0-2.046.963-2.046 2.152 0 .786.843 1.705 1.902 1.782l1.737-3.048Z"
        clipRule="evenodd"
      />
      <path
        fill="#737373"
        d="M14.687 22.176c4.444-.261 8.719-3.107 12.2-8.245A.605.605 0 0 0 27 13.58a.571.571 0 0 0-.104-.335c-1.338-1.977-2.794-3.616-4.33-4.9l-1.06 1.86c.883.76 1.747 1.665 2.583 2.719.301.38.301.932 0 1.311-2.521 3.178-5.299 5-8.064 5.592l-1.338 2.35Z"
      />
    </svg>
  ),
  star: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="14"
      height="14"
      fill="none"
      {...props}
    >
      <path
        fill="#BEBEBE"
        d="m4.802 4.632-4.39.486a.42.42 0 0 0-.35.286c-.053.161 0 .329.116.434C1.483 7.03 3.444 8.813 3.444 8.813c-.002 0-.538 2.594-.895 4.324a.414.414 0 0 0 .612.445c1.536-.873 3.838-2.186 3.838-2.186l3.837 2.187a.42.42 0 0 0 .45-.024.417.417 0 0 0 .163-.42l-.893-4.326 3.266-2.972a.416.416 0 0 0-.233-.72c-1.757-.198-4.39-.49-4.39-.49S8.105 2.218 7.378.608a.416.416 0 0 0-.758 0L4.802 4.632Z"
      />
    </svg>
  ),
  starFilled: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="14"
      height="14"
      fill="none"
      {...props}
    >
      <path
        fill="#F1C40F"
        d="m4.802 4.632-4.39.486a.42.42 0 0 0-.35.286c-.053.161 0 .329.116.434C1.483 7.03 3.444 8.813 3.444 8.813c-.002 0-.538 2.594-.895 4.324a.414.414 0 0 0 .612.445c1.536-.873 3.838-2.186 3.838-2.186l3.837 2.187a.42.42 0 0 0 .45-.024.417.417 0 0 0 .163-.42l-.893-4.326 3.266-2.972a.416.416 0 0 0-.233-.72c-1.757-.198-4.39-.49-4.39-.49S8.105 2.218 7.378.608a.416.416 0 0 0-.758 0L4.802 4.632Z"
      />
    </svg>
  ),
  userProfile: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="250"
      height="250"
      fill="none"
      {...props}
    >
      <rect width="250" height="250" fill="#FF9500" rx="125" />
      <path
        fill="#FAFAFA"
        d="M125 123.5c18.778 0 34-15.222 34-34s-15.222-34-34-34-34 15.222-34 34 15.222 34 34 34ZM194 177.5v8.5a8.44 8.44 0 0 1-2.526 6.01 8.693 8.693 0 0 1-6.099 2.49H64.625a8.69 8.69 0 0 1-6.099-2.49A8.438 8.438 0 0 1 56 186v-8.5c0-13.526 5.452-26.498 15.157-36.062 9.705-9.565 22.868-14.938 36.593-14.938h34.5c13.725 0 26.888 5.373 36.593 14.938C188.548 151.002 194 163.974 194 177.5Z"
      />
    </svg>
  ),
  phone: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="30"
      height="30"
      fill="none"
      {...props}
    >
      <path
        fill="#FF9500"
        d="m29.953 22.676-1.362 5.906a1.821 1.821 0 0 1-1.785 1.42C12.023 30 0 17.976 0 3.192A1.82 1.82 0 0 1 1.42 1.41L7.324.047c.86-.2 1.74.247 2.097 1.062l2.726 6.357a1.834 1.834 0 0 1-.527 2.136l-3.154 2.533a20.488 20.488 0 0 0 9.345 9.346l2.583-3.153a1.823 1.823 0 0 1 2.137-.526l6.358 2.726c.764.402 1.262 1.292 1.063 2.148Z"
      />
    </svg>
  ),
  mail: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="32"
      height="24"
      fill="none"
      {...props}
    >
      <path fill="#FF9500" d="M0 4v20h32V4L16 14 0 4Z" />
      <path fill="#FF9500" d="m0 2 16 10L32 2V0H0v2Z" />
    </svg>
  ),
  userSmall: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="67"
      height="67"
      fill="none"
      {...props}
    >
      <rect width="67" height="67" fill="#FF9500" rx="33.5" />
      <path
        fill="#FAFAFA"
        d="M33.5 32a8.292 8.292 0 1 0 0-16.584A8.292 8.292 0 0 0 33.5 32ZM50.416 47.438v2.073c0 .55-.223 1.077-.62 1.466a2.136 2.136 0 0 1-1.495.607H18.698c-.56 0-1.098-.218-1.495-.607a2.053 2.053 0 0 1-.619-1.466v-2.073c0-3.299 1.337-6.462 3.716-8.795A12.817 12.817 0 0 1 29.27 35h8.459c3.365 0 6.592 1.31 8.97 3.643a12.316 12.316 0 0 1 3.717 8.795Z"
      />
    </svg>
  ),
  upload: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="25"
      height="25"
      fill="none"
      {...props}
    >
      <path
        stroke="#FF9500"
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth="2"
        d="M21.5 15.5v4a2 2 0 0 1-2 2h-14a2 2 0 0 1-2-2v-4M17.5 8.5l-5-5-5 5M12.5 3.5v12"
      />
    </svg>
  ),
  arrowLeftCalendar: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="32"
      height="32"
      fill="none"
      {...props}
    >
      <rect
        width="30"
        height="30"
        x="1"
        y="1"
        fill="#fff"
        rx="15"
      />
      <rect
        width="30"
        height="30"
        x="1"
        y="1"
        stroke="#FF9500"
        strokeWidth="2"
        rx="15"
      />
      <path
        fill="#FF9500"
        d="m19.512 10.898-5 5 5 5-1 2-7-7 7-7 1 2Z"
      />
    </svg>
  ),
  arrowRightCalendar: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="32"
      height="32"
      fill="none"
      {...props}
    >
      <rect
        width="30"
        height="30"
        x="1"
        y="1"
        fill="#fff"
        rx="15"
      />
      <rect
        width="30"
        height="30"
        x="1"
        y="1"
        stroke="#FF9500"
        strokeWidth="2"
        rx="15"
      />
      <path
        fill="#FF9500"
        d="m13.348 21.438 5-5-5-5 1-2 7 7-7 7-1-2Z"
      />
    </svg>
  ),
  trash: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 448 512"
      width="16"
      {...props}
    >
      <path d="M432 80h-82.38l-34-56.75C306.1 8.827 291.4 0 274.6 0H173.4c-16.8 0-32.4 8.827-41 23.25L98.38 80H16C7.125 80 0 87.13 0 96v16c0 8.9 7.125 16 16 16h16v320c0 35.35 28.65 64 64 64h256c35.35 0 64-28.65 64-64V128h16c8.9 0 16-7.1 16-16V96c0-8.87-7.1-16-16-16zM171.9 50.88c1-1.75 3-2.88 5.1-2.88h94c2.125 0 4.125 1.125 5.125 2.875L293.6 80H154.4l17.5-29.12zM352 464H96c-8.837 0-16-7.163-16-16V128h288v320c0 8.8-7.2 16-16 16zm-128-48c8.844 0 16-7.156 16-16V192c0-8.844-7.156-16-16-16s-16 7.2-16 16v208c0 8.8 7.2 16 16 16zm-80 0c8.8 0 16-7.2 16-16V192c0-8.844-7.156-16-16-16s-16 7.2-16 16v208c0 8.8 7.2 16 16 16zm160 0c8.844 0 16-7.156 16-16V192c0-8.844-7.156-16-16-16s-16 7.2-16 16v208c0 8.8 7.2 16 16 16z" />
    </svg>
  ),
};
